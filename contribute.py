...
def main(def_args=sys.argv[1:]):
    ...
    if repository is not None:
        ...
        if end < start:
            repo_name = repository[start:]
        else:
            repo_name = repository[start:end]
        directory = repo_name + '-' + curr_date.strftime('%Y%m%d%H%M%S')
    ...
    os.mkdir(directory)
    for item in ['contribute.py', 'test_contribute.py', 'LICENSE', 'README.md']:
        src = os.path.join(curr_working_dir, item)
        if os.path.exists(src):
            shutil.copy(src, os.path.join(directory, item))
    assets_src = os.path.join(curr_working_dir, 'assets')
    if os.path.exists(assets_src):
        shutil.copytree(assets_src, os.path.join(directory, 'assets'), dirs_exist_ok=True)
    os.chdir(directory)
    run(['git', 'init', '-b', 'main'])
...
def contribute(date):
    with open(os.path.join(os.getcwd(), 'ACTIVITY.log'), 'a') as file:
        file.write(message(date) + '\n\n')
    run(['git', 'add', '.'])
    date_str = date.strftime('%Y-%m-%dT%H:%M:%S')
    run(['git', 'commit', '-m', message(date), '--date', date_str])
